...
def rat_cc(shape: tuple, emb_size: int, n_layers: int, units: int, l: float):
    input_layer = Input(shape=(None, shape[1]), name='input')
    embedding = Bidirectional(LSTM(units, return_sequences=True))(input_layer)
    embedding = Dropout(0.2)(embedding)

    for _ in range(n_layers):
        embedding = Bidirectional(LSTM(units, return_sequences=True))(embedding)
        embedding = Dropout(0.2)(embedding)
    ...
    decoder = Bidirectional(LSTM(units, return_sequences=True))(repeat_layer)
    decoder = Dropout(0.2)(decoder)

    for _ in range(n_layers):
        decoder = Bidirectional(LSTM(units, return_sequences=True))(decoder)
        decoder = Dropout(0.2)(decoder)
